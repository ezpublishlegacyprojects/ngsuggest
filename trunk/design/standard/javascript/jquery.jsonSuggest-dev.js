	...
	$.fn.jsonSuggest = function(searchData, settings) {
		...
		return this.each(function() {
			...
			function selectResultItem(item) {
				//var searchItems = obj.val().split(' ');
				//searchItems.pop();
				//obj.val((searchItems.join(' ') ? searchItems.join(' ') + ' ' : '') + item.text);
				obj.val(item.text);
				$(results).html('').hide();
			...
			function runSuggest() {	
				var strings = obj.val().split(' ');
				if (strings[strings.length - 1].length < settings.minCharacters) {
					$(results).html('').hide();
					return false;
				}
				strings.pop();
				var resultPrepend = strings.join(' ');
				...
				if (settings.ajaxResults === true) {
					resultObjects = searchData(obj.val(), settings.searchId, settings.wildCard, 
				...
				for (var i in resultObjects) {
					resultObjects[i].text = (resultPrepend != '' ? resultPrepend + ' ' : '') + resultObjects[i].text;
				}
				...
				switch (e.keyCode) {
					...
					default:
						if (e.type == 'keyup') {
							...
							timeout = setTimeout(runSuggest, settings.suggestTimeout);
						}
				}
			}
